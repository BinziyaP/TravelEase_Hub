-- Complete Packages Table Creation for TravelEase
-- Using only agency_approval_history table

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

CREATE TABLE IF NOT EXISTS public.packages (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    agency_history_id UUID NOT NULL REFERENCES public.agency_approval_history(id) ON DELETE CASCADE,
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_packages_agency_history_id ON public.packages(agency_history_id);
...
FOR SELECT USING (
    agency_history_id IN (
        SELECT id FROM public.agency_approval_history
...
COMMENT ON COLUMN public.packages.agency_history_id IS 'References agency_approval_history.id to link package to agency approval';
